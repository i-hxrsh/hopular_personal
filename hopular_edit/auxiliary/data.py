...
class CSVDataset(BaseDataset, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.__target_numeric = np.asarray([] if target_numeric is None else target_numeric, dtype=np.longlong)
        self.__target_discrete = np.asarray([] if target_discrete is None else target_discrete, dtype=np.longlong)
        self.__feature_numeric = np.asarray([] if feature_numeric is None else np.union1d(
            np.array(feature_numeric).reshape(-1), np.array(self.__target_numeric).reshape(-1)), dtype=np.longlong)
        self.__feature_discrete = np.asarray([] if feature_discrete is None else np.union1d(
            np.array(feature_discrete).reshape(-1), np.array(self.__target_discrete).reshape(-1)), dtype=np.longlong)
    ...
    def reset(self) -> None:
        ...
        for column_index in range(self.__data.shape[1]):
            ...
            if (len(self.__data_missing_inverse) > 0) and (column_index in self.__data_missing_inverse):
                missing_sample_indices = np.asarray(list(self.__data_missing_inverse[column_index]), dtype=np.longlong)
